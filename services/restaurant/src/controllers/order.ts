...
export const confirmCODOrder = TryCatch(async (req: AuthenticatedRequest, res) => {
  if (!req.user) {
  ...
  res.json({ message: "COD order confirmed", orderId: order._id });
});
...
export const rateOrder = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  const order = await Order.findOne({ _id: orderId as string, userId: req.user._id.toString() });

  if (!order) return res.status(404).json({ message: "Order not found" });
  if (order.status !== "delivered") return res.status(400).json({ message: "Can only rate delivered orders" });
  if (order.rating) return res.status(400).json({ message: "Already rated" });
...
export const getRiderEarnings = TryCatch(async (req, res) => {
  ...
  const totalEarnings = orders.reduce((sum, o) => sum + o.riderAmount, 0);
  const totalOrders = orders.length;

  const byDate: Record<string, { count: number; earnings: number }> = {};
  orders.forEach((o) => {
    const day = new Date(o.createdAt).toLocaleDateString("en-IN", {
      day: "2-digit",
      month: "short",
    });
    if (!byDate[day]) byDate[day] = { count: 0, earnings: 0 };
    byDate[day].count += 1;
    byDate[day].earnings += o.riderAmount;
  });

  res.json({ totalEarnings, totalOrders, byDate });
});
...
export const updateOrderStatusRider = TryCatch(async (req, res) => {
  ...
  if (order.status === "rider_assigned") {
    ...
    // send OTP to customer via Twilio
    try {
      const twilio = (await import("twilio")).default;
      const client = twilio(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN);
      await client.messages.create({
        body: `Your Zomato delivery OTP is ${otp}. Share it with the rider to complete delivery.`,
        from: process.env.TWILIO_PHONE_NUMBER!,
        to: `+91${order.deliveryAddress.mobile}`,
      });
    } catch (e) {
    ...
    await axios.post(`${process.env.REALTIME_SERVICE}/api/v1/internal/emit`,
      { event: "order:rider_assigned", room: `restaurant:${order.restaurantId}`, payload: order },
      { headers: { "x-internal-key": process.env.INTERNAL_SERVICE_KEY } }
    );
    await axios.post(`${process.env.REALTIME_SERVICE}/api/v1/internal/emit`,
      { event: "order:rider_assigned", room: `user:${order.userId}`, payload: order },
      { headers: { "x-internal-key": process.env.INTERNAL_SERVICE_KEY } }
    );

    return res.json({ message: "Order picked up. OTP sent to customer." });
  }
});
...
export const verifyDeliveryOtp = TryCatch(async (req, res) => {
  ...
  if (!order) return res.status(404).json({ message: "Order not found" });
  if (order.status !== "picked_up") return res.status(400).json({ message: "Invalid order status" });
  ...
  await axios.post(`${process.env.REALTIME_SERVICE}/api/v1/internal/emit`,
    { event: "order:update", room: `restaurant:${order.restaurantId}`, payload: { orderId: order._id, status: order.status } },
    { headers: { "x-internal-key": process.env.INTERNAL_SERVICE_KEY } }
  );
  await axios.post(`${process.env.REALTIME_SERVICE}/api/v1/internal/emit`,
