...
export const fetchRiderEarnings = TryCatch(async (req: AuthenticatedRequest, res) => {
  const riderUserId = req.user?._id;
  ...
  const { data } = await axios.get(
    `${process.env.RESTAURANT_SERVICE}/api/order/rider/earnings?riderId=${rider._id}`,
...
export const verifyDeliveryOtp = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  try {
    const { data } = await axios.post(
      `${process.env.RESTAURANT_SERVICE}/api/order/verify/otp`,
      { orderId, otp },
      { headers: { "x-internal-key": process.env.INTERNAL_SERVICE_KEY } }
    );
    await Rider.findOneAndUpdate({ userId }, { isAvailble: true });
