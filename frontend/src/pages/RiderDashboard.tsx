...
const RiderDashboard = () => {
  ...
  const [earnings, setEarnings] = useState<{
    ...
    totalOrders: number;
    byDate: Record<string, { count: number; earnings: number }>;
  } | null>(null);
  ...
  const fetchEarnings = async () => {
    try {
      const { data } = await axios.get(`${riderService}/api/rider/earnings`, {
  ...
  useEffect(() => {
    if (user?.role === "rider") fetchEarnings();
  }, [user]);
        ...
        <div className="mx-auto max-w-md px-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-lg bg-green-50 p-3 text-center">
              <p className="text-lg font-bold text-green-600">₹{earnings.totalEarnings}</p>
              <p className="text-xs text-gray-500">Total Earned</p>
            </div>
            <div className="rounded-lg bg-blue-50 p-3 text-center">
              <p className="text-lg font-bold text-blue-600">{earnings.totalOrders}</p>
              <p className="text-xs text-gray-500">Deliveries</p>
            </div>
          </div>
          <div className="space-y-2">
            {Object.entries(earnings.byDate).slice(0, 7).map(([date, d]) => (
              <div key={date} className="flex justify-between rounded-lg border px-4 py-2 text-sm">
                <span className="text-gray-500">{date}</span>
                <span className="text-gray-500">{d.count} orders</span>
                <span className="font-semibold text-green-600">₹{d.earnings}</span>
              </div>
            ))}
          </div>
        </div>
