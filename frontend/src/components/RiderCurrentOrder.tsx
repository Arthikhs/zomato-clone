...
const RiderCurrentOrder = ({ order, onStatusUpdate }: Props) => {
  ...
  const verifyOtp = async () => {
    ...
    try {
      setVerifying(true);
      await axios.post(
        `${riderService}/api/rider/order/verify-otp`,
        { orderId: order._id, otp },
        { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
      );
    ...
    <div className="rounded-xl bg-white shadow-sm p-4 space-y-4">
      ...
      <div className="space-y-2">
        ...
        {order.status === "picked_up" && (
          <div className="space-y-2">
            <p className="text-sm text-gray-500">Enter OTP from customer to complete delivery</p>
            <input
              type="number"
              maxLength={4}
              ...
              className="w-full rounded-lg border px-4 py-2 text-center text-lg font-bold tracking-widest outline-none"
            />
            <button
              onClick={verifyOtp}
              disabled={verifying}
